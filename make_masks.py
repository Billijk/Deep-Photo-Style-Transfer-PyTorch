...
# encode rgb color into one 32-bit integer
def encode(rgb):
...
colors = {k: i for i, k in enumerate(colors)}

in_seg, tar_seg = [], []
for c in colors:
    in_seg.append(np.float32(mat1 == colors[c]))
    tar_seg.append(np.float32(mat2 == colors[c]))
